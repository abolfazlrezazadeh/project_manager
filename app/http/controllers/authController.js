...
const { hashString, compareDataWithHashedPassword } = require("../../modules/hashString");
...
try {
  ...
  if(!user) throw {status : 401 , success : false , message : " the username or password is incorrect" };
  if(!compareDataWithHashedPassword(password ,user.password)) throw {status : 401 , success : false , message : " the username or password is incorrect" };
  return res.status(200).json({
    status : 200 , 
    success : true , 
